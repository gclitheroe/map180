...
func (m *map3857) labels() (l []label, err error) {
	...
	case true:
		...
		AND zoom = $9
 		...
 		select round(ST_X(pt)), round(ST_Y(pt)*-1), type,name from l`, m.llx, m.lly, left3857, m.ury, m.xshift, m.yshift, m.dx, m.dx, m.zoom)
		if err != nil {
			return
		}
		defer rows.Close()
		...
		AND zoom = $9
 		...
 		select round(ST_X(pt)), round(ST_Y(pt)*-1), type,name from l`, right3857, m.lly, m.urx, m.ury, width3857-m.llx, m.yshift, m.dx, m.dx, m.zoom)
		if err != nil {
			return
		}
		defer rows.Close()
...
func labelsToSVG(labels []label) string {
	...
	for _, l := range labels {
		...
		case 3:
			...
			b.WriteString(fmt.Sprintf("<text fill=\"grey\" font-style=\"italic\" x=\"%d\" y=\"%d\" font-size=\"%d\" text-anchor=\"start\">%s</text>", l.x+3, l.y+5, 11, l.label))
		case 4:
			b.WriteString(fmt.Sprintf("<circle cx=\"%d\" cy=\"%d\" r=\"1\" stroke=\"#6d8383\" stroke-width=\"1\" fill=\"darkslategrey\" />", l.x, l.y))
			b.WriteString(fmt.Sprintf("<text fill=\"#6d8383\" font-style=\"italic\" x=\"%d\" y=\"%d\" font-size=\"%d\" text-anchor=\"start\">%s</text>", l.x+3, l.y+5, 11, l.label))
		}
	}
